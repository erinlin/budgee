...
### Requirement: 高對比色彩系統
系統 SHALL 確保所有文字與背景的對比度符合 WCAG AAA 標準（>= 7:1）。主色 `--color-primary` SHALL 為 `#3d7000`，`--text-muted` SHALL 為 `#555555`。語義化顏色維持不變：紅色系 = 待付餘額/錯誤、綠色系 = 已結清/待退。

#### Scenario: 主色對比度驗證
- **WHEN** 使用者在白色背景上檢視主色文字或按鈕
- **THEN** 主色 `#3d7000` 與白色背景的對比度 >= 7:1

#### Scenario: muted 文字對比度驗證
- **WHEN** 使用者在白色背景上檢視次要文字（如標籤、說明文字）
- **THEN** `--text-muted` (#555555) 與白色背景的對比度 >= 7:1
