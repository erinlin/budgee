...
- [x] 4.1 建立 Member 的列表與新增表單（輸入暱稱、選擇角色）
- [x] 4.2 實作重複暱稱檢查（非強制阻擋，但顯示警告）
- [x] 4.3 實作角色指派（主辦人/會計）與 UI 標籤顯示
- [x] 4.4 實作刪除成員時的檢查（若該成員已有花費紀錄，跳出 ConfirmDialog 警告）
- [x] 4.5 自動將新增的成員暱稱加入跨旅行 localStorage 快取s）
...
- [x] 6.1 建立花費列表頁面（TanStack Table，欄位：日期、類型、標題、總金額、代墊人、分攤人）
- [x] 6.2 建立新增花費 Modal/頁面，根據類型動態切換表單：
  - split：總金額輸入 + 成員多選（含「全選」捷徑）
  - per-item：每位成員各選一個品項（Select）
- [x] 6.3 實作 split 金額計算（totalAmount ÷ 選中人數）
- [x] 6.4 實作 per-item 金額計算（每人 = 其選項 price，total = 加總）
- [x] 6.5 代墊人選擇器（從成員列表選擇，可選「無代墊」）
- [x] 6.6 花費日期選擇器（預設今日，限旅行日期範圍）
- [x] 6.7 備註欄位
- [x] 6.8 實作花費編輯與刪除功能
- [x] 6.9 點選花費列顯示分攤明細（每人名稱、品項、金額）

## 7. 收款管理（collection-tracking）

- [x] 7.1 建立收款頁面：上方為每人餘額摘要表格，下方為收款紀錄列表
- [x] 7.2 實作餘額計算函式（分攤總額 - 代墊總額 - 已收款總額）
- [x] 7.3 建立新增收款紀錄表單（成員、金額、類型 pre-collect/collect、備註）
- [x] 7.4 餘額顯示：正值紅色系（待繳）、負值綠色系（待退）、零值中性色

## 8. 個人預算頁面（personal-budget-view）

- [x] 8.1 建立個人頁面：成員選擇器（Dropdown）
- [x] 8.2 顯示該成員的花費分攤明細表格（花費標題、品項、日期、分攤金額）
- [x] 8.3 顯示該成員的代墊紀錄表格
- [x] 8.4 顯示該成員的收款紀錄表格
- [x] 8.5 顯示餘額摘要（分攤總計、代墊總計、已收總計、最終待收/待退）

## 9. 資料匯出匯入（data-export-import）

- [x] 9.1 實作 JSON 匯出（File API + Blob），檔名格式：`budgee-{title}-{date}.json`
- [x] 9.2 實作 JSON 匯入：解析檔案 → 判斷 `archived` → 決定可編輯或唯讀
- [x] 9.3 匯入 id 衝突處理（提示覆蓋或並存）
- [x] 9.4 實作 PDF 匯出（jsPDF + jsPDF-AutoTable），含繁體中文字型嵌入
- [x] 9.5 PDF 內容：旅行基本資訊 + 花費明細表格 + 每人收款摘要表格

## 10. PWA 離線設定

- [x] 10.1 設定 vite-plugin-pwa（manifest：App 名稱 Budgee、icon、主題色）
- [x] 10.2 設定 Service Worker Workbox 策略（靜態資源 CacheFirst）
- [x] 10.3 驗證離線情境：斷網後重新整理，App 正常顯示且資料完整
- [x] 10.4 App icon 生成（至少 192×192 與 512×512）

## 11. 收尾與驗證

- [x] 11.1 驗證所有花費計算邏輯（split、per-item、餘額計算）正確性
- [x] 11.2 驗證封存後所有編輯入口均不可操作
- [x] 11.3 驗證 JSON 匯出→匯入→再匯出的資料一致性
- [x] 11.4 在行動裝置模擬器上驗證觸控點擊區域（≥ 48×48px）
- [x] 11.5 驗證深色模式切換，確認視覺正確
- [x] 11.6 更新 openspec/config.yaml，加入專案技術棧與慣例說明
